...
class SmallFaceCameraAugmentation:
    """Conservative fixed camera degradation used by the VAC small-face run."""
    ...
    def __call__(self, sample: FaceSample) -> FaceSample:
        ...
        if np.random.random() < 0.15:
            image = cv2.GaussianBlur(image, (3, 3), sigmaX=np.random.uniform(0.3, 0.8))
        if np.random.random() < 0.20:
            quality = int(np.random.randint(75, 96))
        ...
        if np.random.random() < 0.15:
            noise = np.random.normal(0.0, np.random.uniform(1.0, 3.0), image.shape)
            image = np.clip(image.astype(np.float32) + noise, 0, 255).astype(np.uint8)

        sample.image = image
        sample.image_shape = image.shape
        return sample
...
def build_train_transforms(
    ...
    if small_face_policy:
        transforms.append(SmallFaceCameraAugmentation())
    transforms.extend((
