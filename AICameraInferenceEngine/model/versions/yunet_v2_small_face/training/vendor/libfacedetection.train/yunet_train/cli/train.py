...
from yunet_train.engine import LinearWarmupMultiStepLR, load_checkpoint, load_model_weights_only, save_checkpoint
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--small-face-policy", action="store_true")
...
def run_training(args: argparse.Namespace) -> None:
    seed = getattr(args, "seed", 20260905)
    random.seed(seed)
    np.random.seed(seed)
    ...
    if args.resume is not None and init_weights is not None:
        raise ValueError("--resume and --init-weights are mutually exclusive")
    if init_weights is not None:
        load_model_weights_only(init_weights, model=model, map_location="cpu")
        logger(f"initialized_weights path={init_weights}")
    ...
    dataset = WIDERFaceDataset(
        ...
        transform=build_train_transforms(
            ...
            if getattr(args, "small_face_policy", False) else get_train_crop_choice(args.variant),
            min_box_size=args.min_face_size,
            small_face_policy=getattr(args, "small_face_policy", False),
        ),
    )
    ...
    try:
        ...
        if device.type == "cuda":
            logger(
                f"cuda_peak_allocated_mb={torch.cuda.max_memory_allocated(device) / 1024**2:.1f} "
                f"cuda_peak_reserved_mb={torch.cuda.max_memory_reserved(device) / 1024**2:.1f}"
            )
