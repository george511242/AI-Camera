...
class YoloV8Pose(ONNXModel):
    ...
    def NMS(self, detectResult):
        ...
        # convert to arrays
        xs1 = np.array([b.xmin for b in detectResult], dtype=np.float32)
        ys1 = np.array([b.ymin for b in detectResult], dtype=np.float32)
        xs2 = np.array([b.xmax for b in detectResult], dtype=np.float32)
        ys2 = np.array([b.ymax for b in detectResult], dtype=np.float32)
        scores = np.array([b.score for b in detectResult], dtype=np.float32)
        classes = np.array([b.classId for b in detectResult], dtype=np.int32)
