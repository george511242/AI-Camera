...
class MobileNet(ONNXModel):
    ...
    def inference_from_crop(self, img, crop_info):
        ...
        crop = img_utils.z_norm(crop, [0., 0., 0.], [255., 255., 255.])
        crop = img_utils.z_norm(crop, self.imagenet_mean, self.imagenet_std)
        crop = img_utils.channel_first(crop)
        crop = img_utils.to_batch([crop])

        # inference
        predict = self.session.run(None, {'input': crop})
