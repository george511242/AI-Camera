...
class YuNet(RKNNModel):
    ...
    def __call__(self, img: cv2.typing.MatLike, results: 'list[Result]') -> 'list[Result]':
        ...
        # inference
        output = self.session.inference([input_img])
        ...
        for (i, stride), anchor_centers in zip(enumerate(self.strides), self.strides_anchor_centers):
            ...
            # Vectorized: (N, NK*2) -> (N, NK, 2) * stride + anchor_centers -> (N, NK*2)
            per_kps = (kps_pred.reshape(-1, self.NK, 2) * stride + anchor_centers[:, None, :]).reshape(-1, 2 * self.NK)
            kpss.append(per_kps)
