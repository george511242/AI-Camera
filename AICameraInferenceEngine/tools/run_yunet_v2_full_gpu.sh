...
OUT="$ROOT/model/versions/yunet_v2_small_face/training/run1"
WEIGHTS="$TRAINER/weights/yunet_n.pth"

cd "$ROOT"

if [[ -e "$OUT" ]]; then
  echo "Refusing to overwrite existing full-training directory: $OUT" >&2
  echo "Inspect the existing run before deciding whether to resume it." >&2
  exit 2
fi
...
if ! "$PY" -c 'import torch; assert torch.cuda.is_available(); print(torch.cuda.get_device_name(0))'; then
  echo "CUDA is unavailable in vac-yunet-gpu; full training was not started." >&2
  exit 4
fi
...
mkdir -p "$OUT"
{
  ...
  "$PY" -c 'import platform, torch; print(f"python={platform.python_version()}"); print(f"torch={torch.__version__}"); print(f"cuda={torch.version.cuda}"); print(f"gpu={torch.cuda.get_device_name(0)}"); print(f"capability={torch.cuda.get_device_capability(0)}")'
  free -h
  nvidia-smi
} | tee "$OUT/environment.log"

echo "Starting the single YuNet v2 small-face full run in: $OUT"
...
echo "=== host resources after full training ===" | tee -a "$OUT/terminal.log"
free -h | tee -a "$OUT/terminal.log"
nvidia-smi | tee -a "$OUT/terminal.log"
echo "Full-training artifacts: $OUT" | tee -a "$OUT/terminal.log"
